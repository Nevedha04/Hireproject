...
const Home = ({ navigation }) => {
  ...
  const fetchEmployeeCount = async (jobTitle) => {
    try {
      const response = await fetch(`http://192.168.29.225:8080/api/employees/countByJobDomain?jobDomain=${encodeURIComponent(jobTitle)}`);
      const data = await response.json();
      console.log(`Count for ${jobTitle}:`, data);  // Debugging log
      setEmployeeCounts(prevCounts => ({ ...prevCounts, [jobTitle]: data }));
    } catch (error) {
